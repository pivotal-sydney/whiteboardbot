	...
	Context("registering standup", func() {
		Describe("when standup has not been registered", func() {
			It("should ask for standup ID", func() {
				...
				Expect(slackClient.Message).To(Equal(THUMBS_DOWN + "You haven't registered your standup yet. wb r <id> first!"))
			})
		})
		...
		Describe("with an integer as standup id", func() {
			It("should respond registration successful", func() {
				...
				Expect(slackClient.Message).To(Equal(THUMBS_UP + "Standup Sydney has been registered! You can now start creating Whiteboard entries!"))
			})
		})
	})
})
